...
function HttpBackend() {
    ...
    this.fetchFn = function (input, init) {
        ...
        return new _bluebird2.default(function (resolve, reject) {
            ...
            var req = new Request(requestOpts, callback);
...
HttpBackend.prototype = {
     ...
     */
    flush: function flush(path, numToFlush, waitTime) {
        ...
        var _tryFlush = function _tryFlush() {
     ...
     *    number of requests flushed
     */
    flushAllExpected: function flushAllExpected(opts) {
        var _this = this;
        ...
        if (this.expectedRequests.length === 0) {
            ...
            return _bluebird2.default.reject(new Error('flushAllExpected called with an empty expectation list'));
        }

        var waitTime = opts.timeout === undefined ? 1000 : opts.timeout;
        var endTime = waitTime + Date.now();
        var flushed = 0;
        ...
        var iterate = function iterate() {
            ...
            if (timeRemaining <= 0) {
                throw new Error('Timed out after flushing ' + flushed + ' requests; ' + (_this.expectedRequests.length + ' remaining'));
            }

            return _this.flush(undefined, undefined, timeRemaining).then(function (f) {
                flushed += f;

                if (_this.expectedRequests.length === 0) {
                    // we're done
                    return null;
                }
        ...
        var prom = new _bluebird2.default(function (resolve, reject) {
     ...
     */
    _takeFromQueue: function _takeFromQueue(path) {
        var req = null;
        var i = void 0;
        var j = void 0;
        var matchingReq = null;
        var expectedReq = null;
     ...
     */
    verifyNoOutstandingRequests: function verifyNoOutstandingRequests() {
        var firstOutstandingReq = this.requests[0] || {};
        (0, _expect2.default)(this.requests.length).toEqual(0, "Expected no more HTTP requests but received request to " + firstOutstandingReq.path);
    },
    ...
    verifyNoOutstandingExpectation: function verifyNoOutstandingExpectation() {
        ...
        (0, _expect2.default)(this.expectedRequests.length).toEqual(0, "Expected to see HTTP request for " + firstOutstandingExpectation.path);
    },
     ...
     */
    when: function when(method, path, data) {
     ...
     */
    stop: function stop() {
        return _bluebird2.default.all(this._flushPromises);
    }
};
...
ExpectedRequest.prototype = {
     ...
     */
    respond: function respond(code, data, rawBody) {
        this.response = {
            response: {
                statusCode: code,
                headers: {
